...
with tempfile.TemporaryFile(mode="w+t") as tfp:
  tfp.write("This is some temporary data again but with context")
  tfp.seek(0)
  print(tfp.read())

# TODO: create a temporary directory using the TemporaryDirectory class
with tempfile.TemporaryDirectory() as tdp:
  path = os.path.join(tdp, "Impact_1.txt")
  with open(path, mode="w+t") as tfp:
    tfp.write("This is some temporary data again in a temporary dir but with context")
    tfp.seek(0)
    print(tfp.read())